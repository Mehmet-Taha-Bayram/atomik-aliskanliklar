...
st.markdown("""
    ...
    .stCheckbox { font-size: 20px; padding: 10px; background: white; border-radius: 10px; margin-bottom: 5px; box-shadow: 2px 2px 5px rgba(0,0,0,0.05); }
    </style>
...
if st.button("Günü Tamamla ve Kaydet!", use_container_width=True):
    ...
    # %1 Kuralı: Tam yaparsan artar, yapmazsan yerinde sayar
    ...
    if oran == 1.0:
        ...
        st.info("Güzel deneme! Yarın %1 için tekrar odaklan.")

# --- İSTATİSTİKLER ---
st.divider()
c1, c2, c3 = st.columns(3)
c1.metric("Toplam Gün", len(st.session_state.history))
c2.metric("Bugünkü Puan", f"%{oran*100:.0f}")
son_deger = st.session_state.history["Bileşik_Büyüme"].iloc[-1] if len(st.session_state.history) > 0 else 1.0
c3.metric("Toplam Gelişim", f"x{son_deger:.2f}")

# --- GRAFİK ---
if not st.session_state.history.empty:
    st.subheader("📈 Gelişim Yolculuğun")
    fig = px.area(st.session_state.history, x="Gün", y="Bileşik_Büyüme", 
                  title="Bileşik Etki Grafiği", color_discrete_sequence=['#00CC96'])
